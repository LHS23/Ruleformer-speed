...
def run(translator, data_loader, id2r, mode, optimizer, device, padding, epoch, logfile, starttime, decode, scaler=None):
    pred_line = []
    ...
    for i,(subgraph , link, target, tailIndexs, length) in enumerate(data_loader):
        ...
        if mode=='train':
            ...
            # scaler.step 会调用内部优化器 (Adam) 的 step 方法
...
def main(tofetch):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_data, batch_size=opt.batch_size, shuffle=True, collate_fn=pickleDataset.collate_fn, num_workers=16) # , num_workers=16
    ...
    for epoch in range(opt.epoch):
        if not opt.decode_rule:
            run(translator, train_loader, base_data.id2r, 'train', optimizer, device, opt.padding, epoch+1, logfile, starttime, opt.decode_rule, scaler)
